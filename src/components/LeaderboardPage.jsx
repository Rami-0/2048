  ...
  return (
    <main className="leaderboard-page">
      ...
      <section className="ranking-panel">
        ...
        {scores.length > 0 && <ol className="ranking-list">{scores.map((entry, index) => <li key={`${entry.name}-${index}`}><span>{String(index + 1).padStart(2, "0")}</span><b>{entry.country && <i>{countryFlag(entry.country)}</i>}{entry.name}</b><strong>{entry.score.toLocaleString()}</strong></li>)}</ol>}
      </section>
